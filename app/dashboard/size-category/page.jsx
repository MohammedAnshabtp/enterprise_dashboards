...
import {
  useSizeCategories, useCreateSizeCategory,
  useUpdateSizeCategory, useDeleteSizeCategory,
...
export default function SizeCategoryPage() {
  ...
  const { data: result, isLoading, isFetching } = useSizeCategories(params);
  const items      = result?.data       ?? [];
  const pagination = result?.pagination ?? {};

  const create = useCreateSizeCategory();
  const update = useUpdateSizeCategory();
  const del    = useDeleteSizeCategory();
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          <div className="flex items-center gap-3 flex-1">
            <div className="w-10 h-10 rounded-xl bg-[#EEF2FF] flex items-center justify-center shrink-0">
              <Ruler size={18} className="text-[#6366F1]" />
            </div>
            <div>
              <h1 className="text-xl font-bold text-[#0F172A]">Size Categories</h1>
              <p className="text-xs text-[#94A3B8]">
                {pagination.totalItems != null ? `${pagination.totalItems} categories total` : "Manage tile size categories"}
              </p>
            </div>
          </div>
      ...
      <div className={`transition-opacity duration-150 ${isFetching && !isLoading ? "opacity-60" : ""}`}>
        {isLoading ? (
          ...
          <EmptyState icon={Ruler} title="No size categories found" />
        ) : viewMode === "grid" ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {items.map((item) => (
              <div key={item._id}
      ...
      <Dialog open={modalOpen} onOpenChange={setModal}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editItem ? "Edit Size Category" : "Add Size Category"}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-2">
            <div className="space-y-1.5">
              <Label>Category Name <span className="text-red-500">*</span></Label>
              <Input placeholder="e.g. 600×600mm" {...register("name")} />
