...
import {
  useSpaceCategories, useCreateSpaceCategory,
  useUpdateSpaceCategory, useDeleteSpaceCategory,
...
export default function SpaceCategoryPage() {
  ...
  const { data: result, isLoading, isFetching } = useSpaceCategories(params);
  const items      = result?.data       ?? [];
  const pagination = result?.pagination ?? {};

  const create = useCreateSpaceCategory();
  const update = useUpdateSpaceCategory();
  const del    = useDeleteSpaceCategory();
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm px-6 py-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          <div className="flex items-center gap-3 flex-1">
            ...
            <div>
              <h1 className="text-xl font-bold text-[#0F172A]">Space Categories</h1>
              <p className="text-xs text-[#94A3B8]">
                {pagination.totalItems != null ? `${pagination.totalItems} categories total` : "Manage room space categories"}
              </p>
            </div>
          </div>
      ...
      <div className={`transition-opacity duration-150 ${isFetching && !isLoading ? "opacity-60" : ""}`}>
        {isLoading ? (
          ...
          <EmptyState icon={Layers} title="No space categories found" />
        ) : viewMode === "grid" ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {items.map((item) => (
              <div key={item._id}
            ...
            {items.map((item, i) => (
              <div key={item._id}
                ...
                <div className="w-12 h-12 rounded-xl overflow-hidden border border-[#E2E8F0] shrink-0">
                  <img src={item.image?.url || "/placeholder.png"} alt={item.name}
      ...
      <Dialog open={modalOpen} onOpenChange={setModal}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editItem ? "Edit Space Category" : "Add Space Category"}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-2">
            <div className="space-y-1.5">
              <Label>Category Name <span className="text-red-500">*</span></Label>
              <Input placeholder="e.g. Living Room" {...register("name")} />
            ...
            <div className="space-y-1.5">
              ...
              <Input placeholder="e.g. Living room with tile floor" {...register("alt")} />
            </div>
            <div className="space-y-1.5">
