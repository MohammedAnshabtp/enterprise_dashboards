...
import { Upload, Pencil, Trash2, Plus, ChevronLeft, ChevronRight } from "lucide-react";
...
const EMPTY_FORM = { name: "", description: "", alt: "", image: null };

export default function SpaceCategory() {
  const [page, setPage] = useState(1);
  const limit = 6;

  const { data, isLoading } = useSpaceCategories({ page, limit });
  const categories = data?.items || data || [];
  const pagination = data?.pagination || {};

  const createCategory = useCreateSpaceCategory();
  const updateCategory = useUpdateSpaceCategory();
  ...
  const handleEdit = (c) => {
    setEditId(c._id);
    setForm({ name: c.name || "", description: c.description || "", alt: c.alt || "", image: null });
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {isLoading ? (
          [...Array(limit)].map((_, i) => (
            <div key={i} className="bg-white rounded-3xl overflow-hidden border animate-pulse">
          ...
          <div className="col-span-full bg-white rounded-3xl p-16 text-center border">
            ...
            <p className="text-gray-500 mt-2">Create your first space category</p>
          </div>
        ) : (
          categories.map((c) => (
            <div
          ...
          >
            ...
            <div className="px-5 py-4 border-t bg-gray-50 flex justify-end gap-3">
              ...
              <button
                ...
                disabled={isPending}
                className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 rounded-xl disabled:opacity-60"
              >
